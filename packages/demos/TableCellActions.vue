<template>
  <div>
    <table class="tr2 tc5 tr6" aria-label="Users table">
      <thead>
        <tr>
          <FeatherSortHeader
            scope="col"
      ...
      <tbody>
        ...
        >
          ...
          <td>
            <FeatherButton icon="Refresh Something" @click="fakeAction">
              <FeatherIcon :icon="refresh" />
            </FeatherButton>
            <FeatherButton icon="Backup Something" @click="fakeAction">
              <FeatherIcon :icon="backup" />
            </FeatherButton>
            <FeatherButton
              icon="View Source of Something"
              @click.stop="fakeAction"
            >
              <FeatherIcon :icon="code" />
            </FeatherButton>
            <FeatherDropdown>
              <template v-slot:trigger>
                <FeatherButton icon="Item Actions" menu-trigger>
  ...
  methods: {
    ...
    getIcon(status) {
      if (status.toLowerCase() === "online") {
        return markIcon(Available);
      }
      return markIcon(Unavailable);
    },
    updatePageSize(v) {
      this.pageSize = v;
    },
