...
<template>
  <div class="d3-container">
    <FeatherTooltip :title="summary" v-slot="{ attrs, on }">
      <svg
        :id="id"
        @click="clicked"
        class="d3-chart"
...
const props = defineProps({
  ...
  dimensions: {
    type: Object as PropType<FeatherChartD3Dimensions>,
    required: true,
  },
  options: { type: Object as PropType<FeatherChartD3Options>, required: true },
...
const ctrWidth = dim.width - dim.margin.left - dim.margin.right;
const ctrHeight = dim.height - dim.margin.top - dim.margin.bottom;
...
onMounted(() => {
  ...
  // DRAW SVG
  const svg = select(`#${id}`)
    .attr("width", ctrWidth + dimensions.margin.left + dimensions.margin.right)
    .attr(
      "height",
  ...
  svg
    .append("g")
    .classed("yAxis", true) //.transition().duration(1000)
