...
const useTabContainer = (
  ...
  const handleKey = (evt: KeyboardEvent) => {
    ...
    // NOTE:  from keyCode number[] to code string[]
    const nextKeys = [CODES.RIGHT];
    const prevKeys = [CODES.LEFT];
    const selectKeys = [CODES.ENTER, CODES.SPACE];
    if (props.vertical) {
      nextKeys.push(CODES.DOWN);
      prevKeys.push(CODES.UP);
    }

    // NOTE:  using includes since we added code as string
